...
I got the textures from C:\Users\Rohit\AppData\Roaming\.minecraft\versions\21w16a\21w16a.jar (extracted the files to a folder)
...
import os # built-in
files = os.listdir('C:\\Users\\Rohit\\Documents\\21w16a_textures') # This is where I stored the images that I kept
f = open(f"C:\\Users\\Rohit\\Documents\\21w16a_textures\\allBlocks.txt", 'w') # The file where the names and colors will the stored.
for file in files: # Iterate through each png file
    if file.split('.')[1] == "png": # Make sure that non-png files are excluded
        thisImg = Image.open(f"C:\\Users\\Rohit\\Documents\\21w16a_textures\\{file}").convert("RGBA") # Open the file and convert it to RGBA rather than the automatic RGBa (where alpha is ignored and PIL tries to calculate the color without the alpha; Their calculations are pretty bad :/ )
